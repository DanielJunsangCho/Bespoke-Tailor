import { JobData, ResumeAnalysis, ATSReport } from '../types/resume';
...
export class AIResumeService {
  ...
  async analyzeJobDescription(jobData: JobData): Promise<ResumeAnalysis> {
  ...
  async tailorResume(baseResume: any, jobData: JobData, analysis: ResumeAnalysis): Promise<string> {
  ...
  private createJobAnalysisPrompt(jobData: JobData): string {
  ...
  private createResumePrompt(baseResume: any, jobData: JobData, analysis: ResumeAnalysis): string {
  ...
  private async callAI(prompt: string, _type: string): Promise<string> {
    const result = await chrome.storage.sync.get(['aiProvider', 'apiKey']);
    const provider = (result.aiProvider as 'claude' | 'openai') || 'claude';
  ...
  private parseJobAnalysis(response: string): ResumeAnalysis {
    try {
  ...
  private extractAnalysisFromText(text: string): ResumeAnalysis {
  ...
  private extractExperienceLevel(text: string): 'entry' | 'mid' | 'senior' {
  ...
  private formatTailoredResume(aiResponse: string, jobData: JobData): string {
  ...
  private extractResumeText(baseResume: any): string {
  ...
  private getFallbackAnalysis(jobData: JobData): ResumeAnalysis {
  ...
  private getFallbackResume(_baseResume: any, jobData: JobData, analysis: ResumeAnalysis): string {
...
Note: This is a fallback version. For full AI-powered tailoring, please configure your API key in extension settings.`;
  ...
  private determineExperienceLevel(description: string): 'entry' | 'mid' | 'senior' {
  ...
  async generateATSReport(resume: string, jobData: JobData): Promise<ATSReport> {
...
4. ATS-friendly formatting recommendations`;
  ...
  private parseATSReport(response: string): ATSReport {
    return {
  ...
  private extractMissingKeywords(text: string): string[] {
  ...
  private extractCompatibility(text: string): 'low' | 'medium' | 'high' {
    const lowerText = text.toLowerCase();
  ...
  private getFallbackATSReport(): ATSReport {
    return {
      score: 75,
